...
public class NativeBannerScene : MonoBehaviour {
	...
	void OnGUI () {
		...
		if (NativeBannerScene.nativeAd != null) {
			...
			GUI.Label (new Rect (50, 500, 600, 50), NativeBannerScene.nativeAd.getTitle (), titleStyle);
			...
			GUI.Label (new Rect (50, 550, 600, 50), NativeBannerScene.nativeAd.getDescription (), descriptionStyle);
			...
			if (GUI.Button (callToActionRect, NativeBannerScene.nativeAd.getCallToAction (), buttonStyle)) {
